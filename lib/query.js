...
class Query {
  ...
  findAllDepartments() {
    return db.promise().query("SELECT department.name AS Departments FROM department");
  }
  ...
  async updateEmployeeManager(employeesArr, managersArr) {
    ...
    let query = await db.promise().query("UPDATE employee SET `manager_id`=? WHERE `id` = ?", [res.manager, res.updateMe]);
    return query;
  }
}
